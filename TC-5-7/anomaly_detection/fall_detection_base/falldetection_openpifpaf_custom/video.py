...
warnings.filterwarnings("ignore", message=".*align.*", module="torch.serialization")
...
def inference(args, stream):
    ...
    ### MODIFIED ###
    if os.path.exists("fall_detector2.pt"):
        fall_detector = FallDetector()
        fall_detector.load_state_dict(torch.load("fall_detector2.pt", map_location="cuda", weights_only=True))
        fall_detector = fall_detector.cuda()
        ...
        keypoints = torch.tensor(keypoints, dtype=torch.float32)[None].cuda()
        with torch.no_grad():
