...
class MPIIVideos():
    DATA_ROOT = "/data/kiat/mpii_videos"
    ...
    def get_frame_list(self, split, video_name):
        data_dir = f"{self.DATA_ROOT}/{video_name.split('-')[0]}/{video_name.split('-')[1]}/"
        print(data_dir)
        data_list = []
    ...
    def get_frame(self, split, video_name, file_name):
        data_path = f"{self.DATA_ROOT}/{video_name.split('-')[0]}/{video_name.split('-')[1]}/{file_name}"
